...
A \textbf{physical episode} is a bounded piece of the actual evolution of a physical system. For computational purposes, the episode should not be reduced to a sequence of global snapshots. What matters is its internal actual organization: events, value-bearing carriers or tokens, transport, consumption, production, routing, control, reuse, and partial order.
...
\begin{definitionbox}
An \textbf{occurrent computational abstraction} is a structure-preserving compression of an actual valued computational provenance structure under a coherent representation scheme. It may hide internal details, merge events, or replace a detailed subsystem by an abstract component, but only when the retained value flow, participation structure, ordering, and composition are genuinely supported by what occurred.
\end{definitionbox}
...
\begin{center}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash\bfseries}p{33mm}XX}
\toprule
...
The observer may decide to ask about $C$, but does not create $C$ by selecting a level. The system either contains the required computational structure or it does not.
\end{thesisbox}
...
\begin{center}
\begin{tabularx}{\textwidth}{>{\bfseries}p{28mm}XX}
\toprule
...
A modal realization, when actually executed, should produce a corresponding occurrent realization under the same representation scheme. But a single occurrent realization generally admits many possible modal completions. The occurrent trace can constrain those completions by accumulating partial operation tables without selecting values for cases that never occurred.
...
A computation $A$ \textbf{refines} a computation $B$ when $A$ retains all the distinctions of $B$ and additional ones. Replacing a loop by a black-box ``SUM'' component moves from a finer member of the spectrum to a coarser one. Abstracting a different component gives a different branch, not necessarily a higher or lower point on one scalar ladder.
...
\textbf{First: physical realization.} Whether an admissible computational structure occurs in the spectrum is intended to be an objective fact about the system. Admissibility includes coherent representation: a computational value cannot be rescued by changing the codebook opportunistically from one occurrence to the next.
...
This locates conventionality in the question rather than in the physical realization. A name may fail to specify its target precisely; that does not imply that membership in the spectrum is observer-relative.
...
Independent local abstractions generate incomparable descriptions. A theory that represents granularity by one scalar level discards real structure in the space of computational descriptions.
\end{consequencebox}

\begin{consequencebox}[title={7. Modal realization projects to occurrent realization}]
Whenever a modal mechanism is actually run, the realized episode should instantiate an occurrent member compatible with that mechanism. The strong notion therefore entails a corresponding weak actual-run notion.
\end{consequencebox}
...
\begin{enumerate}
  \item \textbf{Represent actual processes richly.} Use events and value-bearing tokens with actual transport, consumption, production, ordering, control, reuse, and composition. Do not begin from atomic state sequences.
  \item \textbf{Fix coherent value representation.} Allow heterogeneous local encodings, but require stable typed decoding rules and explicit translations. Do not let the decoder depend on the completed trajectory.
  ...
  \item \textbf{Define admissible abstractions.} An abstraction may forget internal distinctions only when the retained computational organization is genuinely supported by the physical organization and representation scheme.
  \item \textbf{Order abstractions by refinement.} Allow different subsystems to be abstracted independently, giving a poset and, where justified, lattice-like structure.
  ...
  \item \textbf{Relate the two.} Modal structures project to actual runs; actual runs and their partial laws constrain but generally do not uniquely determine modal completions.
...
A computational functionalism should therefore assign phenomenal properties primarily to \textbf{actually instantiated computational episodes}, or to equivalence classes of such episodes under consciousness-irrelevant redescription. The modal computational spectrum remains important for what the system can do, but it need not be constitutive of what is experienced in one actual episode.
...
Differences confined to unrealized computational alternatives should not, by themselves, constitute phenomenal differences in the actual episode.
\end{principlebox}

This principle is stronger than merely saying that the system's observable input-output behavior is the same. The actual internal generative process may still matter. What the principle excludes is a phenomenal dependence on differences whose only manifestation lies in unactualized transitions.
...
Adding dormant branches, disconnected backup hardware, unused lookup tables, or other structures that do not participate in the actual consciousness-relevant process should not alter the phenomenology of that episode.
\end{principlebox}
...
Consciousness-making properties should be invariant under admissible redescriptions and refinements that add only consciousness-irrelevant detail. Multiple spectral descriptions of one physical realization do not by themselves imply multiple phenomenal subjects or episodes.
\end{principlebox}
...
\begin{enumerate}
  ...
  \item $P$ does not change merely because unrealized transition rules, dormant branches, or inactive backup structures are altered.
  ...
  \item $P$ is stable across admissible abstraction and refinement when the added or removed distinctions are declared consciousness-irrelevant.
...
The least conceptually loaded form of computational functionalism is an \textbf{actual-process computational functionalism}: phenomenal facts supervene on invariants of the computation actually generated during an episode, modulo coherent recoding, substrate changes, harmless refinement, and other computationally inert transformations. Modal structure describes the mechanism's capacities and constrains possible episodes, but differences confined to unrealized alternatives do not by themselves alter the phenomenology of the episode that actually occurred.
\end{thesisbox}
...
\textbf{The computational theory} tells us what structures are objectively realized: provenance, representation, abstractions, partial operations, optional support structures, modal mechanisms, spectra, refinement, containment, and common computational cores.
...
\textbf{Physical systems support structured spectra of computational abstractions grounded in coherent representation and actual provenance; modal structure strengthens this into claims about alternative cases, while a disciplined computational functionalism should place present phenomenology primarily on invariants of the actually generated process and treat unrealized counterfactual differences as non-constitutive unless independently justified.}
}}
\end{center}
