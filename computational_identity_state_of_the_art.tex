...
At the occurrent level, the package also distinguishes computational event identity from persistent hardware identity. Reusing one physical device for several operation occurrences and replacing those uses by several devices can preserve the same computational occurrence when represented values, port roles, and dependencies are unchanged. Hardware persistence remains relevant when it changes control, timing, accessible state, or another retained computational relation, but it is not itself a primitive computational difference.
...
\begin{takeawaybox}
The companion conceptual note explores a more actualist default than strongly counterfactual functionalism: present phenomenology is sought primarily among invariants of the computation actually generated during the episode. Modal structure remains crucial for capacities and may be made constitutive when independently motivated; the defeasible default is only that off-trajectory differences are not automatically phenomenal differences.
\end{takeawaybox}
...
Consciousness use & Organizational invariance, functionalism, ICF, IIT-related debates & Naturality desiderata and a candidate actual-process schema, especially a defeasible counterfactual-irrelevance default\\
\bottomrule
\end{tabularx}
\end{center}
...
\begin{enumerate}
  ...
  \item the ontology motivates, without entailing, a candidate family of actual-process functionalist invariances.
\end{enumerate}
