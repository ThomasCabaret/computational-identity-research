...
\[
P=(E,T,\Theta,K,\tau,\kappa,\Pi^{\mathrm{in}},\Pi^{\mathrm{out}},I,O,\prec)
\]
where:
\begin{itemize}
  ...
  \item $K$ is a set of persistent component identities and $\kappa:E\to K$ assigns each event occurrence to the component that performs it;
...
The bipartite graph induced by $I$ and $O$ is the \emph{provenance graph}. A token may fan out to several event ports; an event may have several inputs and outputs. Port labels make local tuples and their permutations explicit, while $\kappa$ distinguishes repeated uses of one component from one-off events performed by different components. The order $\prec$ allows additional causal/temporal precedence without forcing all concurrent events into one total order.
...
\[
C=(E_C,T_C,\Theta_C,K_C,\tau_C,\kappa_C,
\Pi_C^{\mathrm{in}},\Pi_C^{\mathrm{out}},I_C,O_C,\prec_C,\nu_C)
\]
together with its component identities, port families, and optional event labels $\lambda:E_C\to\Lambda$ that identify only the event information licensed at the occurrent level. At minimum, an event label may record the actual port-ordered input-output tuple. A total operation type is not required.
\end{definition}
...
Let $P$ and $C$ be typed valued provenance structures. A \emph{provenance homomorphism} $h:P\to C$ consists of maps on events, tokens, component identities, and event-local ports such that:
\begin{enumerate}
  ...
  \item component identities and any event labels declared structurally relevant are preserved through their declared maps.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item identifications inside a fiber satisfy the selected admissibility policy, which may additionally require connected fibers, boundary preservation, or component compatibility.
\end{enumerate}
...
For a component identity $k\in K$, collect the actual port-ordered input-output tuples observed across events with $\kappa(e)=k$. The port structure is what makes two repeated tuples comparable rather than equal only up to an unspecified permutation.
...
\]
with unfolding-compatible modal realization projecting to occurrent realization on each run, spectra ordered by admissible abstraction after poset reflection, and computational functionalism represented by invariant maps on the witness/spectral structure of actual episodes rather than by arbitrary state labeling or bare input-output equality.
\end{summarybox}
